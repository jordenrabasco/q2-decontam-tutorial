...
# Preamble
This tutorial is intended as an addendum to the base qiime-decontam-tutorial that can be found [here](https://github.com/jordenrabasco/q2-decontam-tutorial/blob/main/Q2_Decontam_Tutorial.html)\
...
# Structure
The pipeline action `decontam-identify-batches` initially splits a feature table into multiple differing subset tables based on the categories in a metadata column provided by the user. For example, to compare contamination across sequencing batches it maybe useful to split the table based on sequencing batch. Then on each of the subset tables decontam processes are run and a comprehensive visual is produced. This action will take the place of `decontam-identify` and `decontam-score-viz` in the base qiime-dceontam-tutorial. 
