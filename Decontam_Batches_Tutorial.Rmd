...
# Preamble
This tutorial is intended as an addendum to the base q2-decontam-tutorial that can be found [here](https://github.com/jordenrabasco/q2-decontam-tutorial/blob/main/Q2_Decontam_Tutorial.html)\
...
# Structure
The pipeline action `decontam-identify-batches` initially splits a feature table multiple differing sub-tables based on the categories in a metadata column provided by the user. For example, to compare contamination across sequencing batches it maybe useful to split the table based on sequencing batch. Then on each of the sub-tables decontam processes are run and a comprehensive visual is produced. This action will take the place of `decontam-identify` and `decontam-score-viz` in the base q2-dceontam-tutorial. 
...
The `decontam-identify-batches` function has the inputs and parameters for all both the `decontam-identify` action and the `decontam-score-viz` functions as well as the options for `feature-tables split`. Those inputs and parameters are;\
